import type { Chapter } from './types'

// German content for chapter 21
export const chapter21De: Chapter = {
  "slug": "chapter-21",
  "number": "21",
  "title": "Männliches und weibliches erektiles System",
  "sections": [
    {
      "id": "presentation",
      "title": "Überblick",
      "blocks": [
        {
          "type": "para",
          "text": "Sexuelle Erregung führt zur Erektion der erektilen Organe, nämlich Klitoris und Penis, sowie zur vaginalen Lubrikation und ermöglicht so die Kopulation."
        }
      ]
    },
    {
      "id": "situation",
      "title": "Lage",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Die äußeren Geschlechtsorgane von Mann und Frau setzen an der Unterfläche des urogenitalen Dreiecks des Perineums an, mit dem sie eng verbunden sind."
          ]
        }
      ]
    },
    {
      "id": "anatomie",
      "title": "Anatomie",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erektile Organe:",
            "Corpora cavernosa: paarige Schwellkörper; sie setzen proximal am Schambogen an. Sie werden als Schenkel der Klitoris bzw. des Penis bezeichnet. An ihren distalen Enden vereinigen sie sich und bilden bei der Frau den Körper der Klitoris und beim Mann die dorsalen Anteile des Penis.",
            "Bulbi vestibuli: Bei der Frau handelt es sich um zwei erektile Organe beidseits des Vaginalorifiziums. Sie beteiligen sich an der erektilen Blutfüllung der Vulva und sind dem männlichen Corpus spongiosum homolog, ohne direkt die Glans der Klitoris zu bilden.",
            "Corpus spongiosum: Beim Mann ist es unpaar, medial gelegen und beginnt mit einer Auftreibung, dem Bulbus penis, der an der Perinealmembran befestigt ist. Es endet in der Glans. Es enthält die Harnröhre. Es trägt dazu bei, den Inhalt der Urethra beim Wasserlassen und bei der Ejakulation des Samens auszutreiben.",
            "Muskeln der erektilen Organe:",
            "Musculi ischiocavernosi: paarige Muskeln; sie setzen am medialen Rand der Rami ischiopubici sowie an den Schenkeln von Klitoris und Penis an. Durch ihre Kontraktion treiben sie Blut aus den Schenkeln in den Körper von Penis und Klitoris.",
            "Musculi bulbospongiosi: paarige Muskeln; sie inserieren am Perineum, bei der Frau am Bulbus vestibuli und beim Mann am Corpus spongiosum. Durch ihre Kontraktion treiben sie Blut in Richtung Penis und Klitoris. Beim Mann erleichtern ihre reflektorischen Kontraktionen die Entleerung der Harnröhre nach der Miktion sowie nach der Ejakulation.",
            "Bänder des Penis: Der Penis ist durch zwei Bänder befestigt:",
            "Ligamentum suspensorium: Es ist kranial an der Symphysis pubica und an der Linea alba der vorderen Bauchwand befestigt;",
            "Ligamentum fundiforme: Es ist an der Linea alba befestigt.",
            "Bartholin-Drüsen, vestibuläre Drüsen: Bei der Frau liegen sie im Perineum, dorsal der Bulbi vestibuli. Sie tragen zur Lubrikation des Scheideneingangs bei. Die vaginale Lubrikation hängt während sexueller Erregung außerdem von der vaskulären Transsudation durch die Vaginalwand ab.",
            "Cowper-Drüsen, bulbourethrale Drüsen: Beim Mann liegen sie im Bulbus des Perineums. Sie sezernieren eine alkalische Flüssigkeit, die die Spermatozoen während der sexuellen Erregung bei ihrem Durchtritt durch Urethra und Vagina schützt."
          ]
        }
      ]
    },
    {
      "id": "vascularisation",
      "title": "Vaskularisation",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Arteriell: Endäste der Arteriae pudendae internae, die ihrerseits vordere Äste der Arteriae iliacae internae sind. (Abbildung 17.3)",
            "Venös: Die Venae pudendae internae begleiten die Arteriae pudendae internae und drainieren überwiegend in das System der Vena iliaca interna. In der klinischen Lesart der ROP können venöse Stauungen im kleinen Becken berücksichtigt werden; die venöse Drainage der erektilen Organe darf jedoch nicht direkt dem hepatischen Pfortadersystem gleichgesetzt werden."
          ]
        }
      ]
    },
    {
      "id": "innervation",
      "title": "Innervation",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Somatische Innervation: Der Nervus pudendus ist der Hauptnerv des Perineums. Er vermittelt die Sensibilität der äußeren Geschlechtsorgane und die somatisch-motorische Steuerung der quergestreiften perinealen Muskulatur, insbesondere der Musculi bulbospongiosi und ischiocavernosi.",
            "Autonome Innervation: (Abbildung 4.26)",
            "Sakrales parasympathisches System S2 bis S4: Es beteiligt sich an der Vasodilatation und an der Füllung der Schwellkörper;",
            "Thorakolumbales sympathisches System L1 bis L3: Es ist insbesondere an der Emissionsphase der Ejakulation beteiligt."
          ]
        }
      ]
    },
    {
      "id": "physiologie",
      "title": "Physiologie",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erektion: Unter dem Einfluss des sakralen Parasympathikus S2 bis S4 steigert die Vasodilatation der Arteriae pudendae internae und ihrer Äste den Blutzufluss in die Schwellkörper von Penis und Klitoris. Bei der Frau geht sexuelle Erregung außerdem mit vulvovaginaler Kongestion und vaginaler Lubrikation einher.",
            "Ejakulation: Sie umfasst eine überwiegend sympathisch vermittelte Emissionsphase, in der die Sekrete der Genitalwege in Richtung der prostatischen Urethra fortgeleitet werden, gefolgt von einer Expulsionsphase, an der insbesondere rhythmische Kontraktionen der Musculi bulbospongiosi unter somatischer Kontrolle des Nervus pudendus beteiligt sind.",
            "Orgasmus: Wirkung des Sympathikus bei Frau und Mann."
          ]
        }
      ]
    },
    {
      "id": "pathologies-courantes",
      "title": "Häufige Pathologien",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Gonadaler Mangel: verminderte Fertilität, Abnahme der Libido, Dysmenorrhö sowie vorzeitige Prämenopause oder Andropause (siehe Kapitel 5, Stressmechanismus, Seite ...).",
            "Beim Mann: Sexuelle Störungen sind zahlreich und häufig: Libidoverlust, sexuelle Versagenssituationen, Erektionsstörungen, vorzeitige Ejakulation, Unfähigkeit zur Ejakulation und Schmerzen beim Geschlechtsverkehr. (siehe Kapitel 16, Nieren, venöse Vaskularisation, linke Niere, Seite ...)",
            "Bei der Frau: Sexuelle Störungen sind ebenfalls zahlreich und häufig: mangelndes sexuelles Interesse (niedrige Libido), vaginale Trockenheit, Schmerzen beim Geschlechtsverkehr, Vaginismus (schmerzhafte Spasmen der Vaginalmuskulatur) und Anorgasmie.",
            "Organische Ursachen: Diabetes, Hyperlipidämie, arterielle Hypertonie, Koronarinsuffizienz, Schlaganfall und Adipositas.",
            "Psychoemotionale Ursachen: siehe viszeroemotionale Beziehungen."
          ]
        },
        {
          "type": "xref",
          "label": "Siehe den Verweis in Kapitel 5",
          "text": "Stressmechanismus",
          "href": "/lecture/chapitre-5?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#p-presentation-0"
        },
        {
          "type": "xref",
          "label": "Siehe den Verweis in Kapitel 16",
          "text": "Nieren, venöse Drainage, linke Niere",
          "href": "/lecture/chapitre-16?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#sec-vascularisation"
        }
      ]
    },
    {
      "id": "relations-viscero-somatiques",
      "title": "Viszerosomatische Beziehungen",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Sakrum, Kokzyx"
          ]
        }
      ]
    },
    {
      "id": "relations-viscero-emotionnelles",
      "title": "Viszeroemotionale Beziehungen",
      "blocks": [
        {
          "type": "para",
          "text": "(Siehe die Kapitel über die weiblichen Geschlechtsorgane, Seite ..., und die männlichen Geschlechtsorgane, Seite ...)"
        },
        {
          "type": "para",
          "text": "Es ist nicht immer leicht, körperliche von psychoemotionalen Ursachen zu unterscheiden, doch letztere sind bei beiden Geschlechtern zahlreich und häufig."
        },
        {
          "type": "para",
          "text": "Beim Mann, und noch mehr bei der Frau, vollzieht sich Sexualität ebenso sehr im Kopf und im Herzen wie im Körper. Psychoemotionale Belastungen sind Hindernisse für eine erfüllte Sexualität: Verlust des Selbstwertgefühls, Angst, Stress, Versagensangst, Depression, Konflikte innerhalb der Partnerschaft, religiöse oder erzieherische Tabus sowie in der Vergangenheit erlebte sexuelle Traumata, darunter Inzest, Vergewaltigung, Berührungsübergriffe und verwandte Erfahrungen."
        }
      ]
    },
    {
      "id": "zones-reflexes-podales",
      "title": "Fußreflexzonen",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Nervus pudendus (S2 bis S4): laterale Fersenfläche, sakraler Plexus, Spina ischiadica; (Abbildungen 17.15 und 17.16),",
            "Plantare Fersenfläche:",
            "Nervus pudendus (Fortsetzung): medialer Rand des Ramus ischiopubicus",
            "Corpora cavernosa und Musculi ischiocavernosi: medialer Rand der Rami ischiopubici;",
            "Erektile Organe: mediales Ende des Ramus iliopubicus (vorderer Fersenrand);",
            "Ligamenta sacrotuberale und sacrospinale: hinterer Fersenrand; (Abbildung 17.21)",
            "Limbisches Gehirn; (Abbildung 3.7)",
            "Nervus olfactorius I. (Abbildung 3.7)"
          ]
        }
      ]
    }
  ]
}
