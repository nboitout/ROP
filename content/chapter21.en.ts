import type { Chapter } from './types'

// English content for chapter 21
export const chapter21En: Chapter = {
  "slug": "chapter-21",
  "number": "21",
  "title": "Male and Female Erectile System",
  "sections": [
    {
      "id": "presentation",
      "title": "Overview",
      "blocks": [
        {
          "type": "para",
          "text": "Sexual arousal causes erection of the erectile organs, namely the clitoris and penis, as well as vaginal lubrication, allowing copulation."
        }
      ]
    },
    {
      "id": "situation",
      "title": "Location",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "The external genital organs of both men and women attach to the inferior surface of the urogenital triangle of the perineum, with which they are intimately connected. (figure 21.1)"
          ]
        }
      ]
    },
    {
      "id": "anatomie",
      "title": "Anatomy",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erectile organs: (figure 21.1)",
            "Corpora cavernosa: paired erectile organs, they attach proximally to the pubic arch. They are known as the crura of the clitoris or penis. At their distal ends, they unite to form the body of the clitoris in women and the dorsal parts of the penis in men.",
            "Bulbs of the vestibule: in women, these are two erectile organs located on either side of the vaginal orifice. They participate in erectile engorgement of the vulva and are homologous to the male corpus spongiosum, without directly forming the glans of the clitoris.",
            "Corpus spongiosum: in men, it is unpaired, medial, and begins as an enlargement, the bulb of the penis, attached to the perineal membrane. It ends in the glans. It contains the urethra. It contributes to expelling urethral contents during micturition and during ejaculation of semen in men.",
            "Muscles of the erectile organs: (figure 21.2)",
            "Ischiocavernosus muscles: paired muscles, they attach to the medial border of the ischiopubic rami and to the crura of the clitoris and penis. When they contract, they propel blood from the crura toward the body of the penis and clitoris.",
            "Bulbospongiosus muscles: paired muscles, they insert onto the perineum, the bulb of the vestibule in women, and the corpus spongiosum in men. When they contract, they drive blood toward the penis and clitoris. In men, their reflex contractions facilitate emptying of the urethra after micturition and ejaculation of semen.",
            "Ligaments of the penis: the penis is attached by two ligaments:",
            "Suspensory ligament: it is attached superiorly to the pubic symphysis and to the linea alba of the anterior abdominal wall; (figure 21.2)",
            "Fundiform ligament: it is attached to the linea alba.",
            "Bartholin vestibular glands: in women, they are located in the perineum, posterior to the vestibular bulbs. They contribute to lubrication of the vaginal introitus. Vaginal lubrication also depends on vascular transudation through the vaginal wall during sexual arousal.",
            "Cowper bulbourethral glands: in men, they are located in the bulb of the perineum. They secrete an alkaline fluid that protects the spermatozoa during their passage through the urethra and vagina during sexual arousal."
          ]
        }
      ]
    },
    {
      "id": "vascularisation",
      "title": "Vascular supply",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Arterial: terminal branches of the internal pudendal arteries, themselves anterior branches of the internal iliac arteries. (figure 17.3)",
            "Venous: the internal pudendal veins accompany the internal pudendal arteries and drain mainly into the internal iliac system. In the clinical interpretation used in ROP, pelvic venous congestion may be taken into account, but venous drainage of the erectile organs should not be directly equated with the hepatic portal system."
          ]
        }
      ]
    },
    {
      "id": "innervation",
      "title": "Innervation",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Somatic innervation: the pudendal nerve is the principal nerve of the perineum. It provides sensation to the external genital organs and somatic motor control of the striated perineal muscles, especially the bulbospongiosus and ischiocavernosus muscles. (figures 21.3 and 21.4)",
            "Autonomic innervation: (figures 4.26 and 21.5)",
            "Sacral parasympathetic system S2 to S4: it participates in vasodilation and filling of the erectile bodies;",
            "Thoracolumbar sympathetic system L1 to L3: it is involved in particular in the emission phase of ejaculation."
          ]
        }
      ]
    },
    {
      "id": "physiologie",
      "title": "Physiology",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erection: under the influence of the sacral parasympathetic system S2 to S4, vasodilation of the internal pudendal arteries and their branches increases blood inflow into the erectile bodies of the penis and clitoris. In women, sexual arousal is also accompanied by vulvovaginal congestion and vaginal lubrication.",
            "Ejaculation: it includes an emission phase, predominantly sympathetic, with progression of secretions from the genital tract toward the prostatic urethra, followed by an expulsion phase, which notably involves rhythmic contractions of the bulbospongiosus muscles under somatic pudendal control.",
            "Orgasm: sympathetic action in both women and men."
          ]
        }
      ]
    },
    {
      "id": "pathologies-courantes",
      "title": "Common pathologies",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Gonadal deficiency: reduced fertility, decreased libido, dysmenorrhea, and early premenopause or andropause (see Chapter 5 Stress Mechanism, page…)",
            "In men: sexual disorders are numerous and frequent: loss of libido, sexual failure, erection problems, premature ejaculation, inability to ejaculate, and pain during intercourse. (see Chapter 16 Kidneys, Venous Vascular Supply, Left Kidney, page…)",
            "In women: sexual disorders are also numerous and frequent: lack of sexual interest (low libido), vaginal dryness, pain during intercourse, vaginismus (painful spasms of the vaginal muscles), and anorgasmia.",
            "Organic causes: diabetes, hyperlipidemia, arterial hypertension, coronary insufficiency, stroke, and obesity.",
            "Psychoemotional causes: see Visceroemotional relationships."
          ]
        },
        {
          "type": "xref",
          "label": "See the reference in chapter 5",
          "text": "Stress mechanism",
          "href": "/lecture/chapitre-5?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#p-presentation-0"
        },
        {
          "type": "xref",
          "label": "See the reference in chapter 16",
          "text": "Kidneys, venous drainage, left kidney",
          "href": "/lecture/chapitre-16?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#sec-vascularisation"
        }
      ]
    },
    {
      "id": "relations-viscero-somatiques",
      "title": "Viscerosomatic relationships",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Sacrum, coccyx"
          ]
        }
      ]
    },
    {
      "id": "relations-viscero-emotionnelles",
      "title": "Visceroemotional relationships",
      "blocks": [
        {
          "type": "para",
          "text": "(See the chapters on the female genital organs, page… and male genital organs, page…)"
        },
        {
          "type": "para",
          "text": "It is not always easy to distinguish physical from psychoemotional causes, but the latter are numerous and frequent in both sexes."
        },
        {
          "type": "para",
          "text": "In men, and even more so in women, sexuality takes place as much in the mind and heart as in the body. Psychoemotional stress is an obstacle to fulfilled sexuality: loss of self-esteem, anxiety, stress, performance anxiety, depression, conflicts within the couple, religious or educational taboos, and past sexual trauma, including incest, rape, inappropriate touching, and related experiences."
        }
      ]
    },
    {
      "id": "zones-reflexes-podales",
      "title": "Foot reflex zones",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Pudendal nerve (S2 to S4): lateral surface of the heel, sacral plexus, ischial spine; (figures 17.15 and 17.16),",
            "Plantar surface of the heel: (figure 21.6)",
            "Pudendal nerve (continued): medial border of the ischiopubic ramus (figure 21.7)",
            "Corpora cavernosa and ischiocavernosus muscles: medial border of the ischiopubic rami;",
            "Erectile organs: medial end of the iliopubic ramus (anterior border of the heel);",
            "Sacrotuberous and sacrospinous ligaments: posterior border of the heels; (figure 17.21)",
            "Limbic brain; (figure 3.7)",
            "Olfactory nerve I. (figure 3.7)"
          ]
        }
      ]
    }
  ]
}
