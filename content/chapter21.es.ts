import type { Chapter } from './types'

// Capítulo 21 — versión en español
export const chapter21Es: Chapter = {
  "slug": "chapter-21",
  "number": "21",
  "title": "Sistema eréctil masculino y femenino",
  "sections": [
    {
      "id": "presentation",
      "title": "Descripción general",
      "blocks": [
        {
          "type": "para",
          "text": "La excitación sexual provoca la erección de los órganos eréctiles, es decir, del clítoris y del pene, así como la lubricación vaginal, lo que permite la cópula."
        }
      ]
    },
    {
      "id": "situation",
      "title": "Situación",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Los órganos genitales externos del hombre y de la mujer se fijan a la cara inferior del triángulo urogenital del periné, con el que están estrechamente relacionados."
          ]
        }
      ]
    },
    {
      "id": "anatomie",
      "title": "Anatomía",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Órganos eréctiles:",
            "Cuerpos cavernosos: órganos eréctiles pares que se insertan proximalmente en el arco púbico. Se denominan pilares del clítoris o del pene. En sus extremos distales se unen para formar el cuerpo del clítoris en la mujer y las porciones dorsales del pene en el hombre.",
            "Bulbos del vestíbulo: en la mujer, son dos órganos eréctiles situados a ambos lados del orificio vaginal. Participan en la congestión eréctil de la vulva y son homólogos al cuerpo esponjoso masculino, sin formar directamente el glande del clítoris.",
            "Cuerpo esponjoso: en el hombre, es una estructura impar y mediana que comienza como un engrosamiento, el bulbo del pene, fijado a la membrana perineal. Termina en el glande. Contiene la uretra. Contribuye al vaciamiento uretral durante la micción y durante la eyaculación del semen.",
            "Músculos de los órganos eréctiles:",
            "Músculos isquiocavernosos: músculos pares que se insertan en el borde medial de las ramas isquiopubianas y en los pilares del clítoris y del pene. Al contraerse, impulsan la sangre desde los pilares hacia el cuerpo del pene y del clítoris.",
            "Músculos bulboesponjosos: músculos pares que se insertan en el periné, en el bulbo del vestíbulo en la mujer y en el cuerpo esponjoso en el hombre. Al contraerse, desplazan la sangre hacia el pene y el clítoris. En el hombre, sus contracciones reflejas facilitan el vaciamiento de la uretra tras la micción y la eyaculación.",
            "Ligamentos del pene: el pene está fijado por dos ligamentos:",
            "Ligamento suspensorio: se fija superiormente a la sínfisis púbica y a la línea alba de la pared abdominal anterior;",
            "Ligamento fundiforme: se fija a la línea alba.",
            "Glándulas vestibulares mayores de Bartolino: en la mujer, se sitúan en el periné, por detrás de los bulbos vestibulares. Participan en la lubricación del introito vaginal. La lubricación vaginal también depende de la trasudación vascular a través de la pared vaginal durante la excitación sexual.",
            "Glándulas bulbouretrales de Cowper: en el hombre, se sitúan en el bulbo del periné. Secretan un líquido alcalino que protege a los espermatozoides durante su paso por la uretra y la vagina en el curso de la excitación sexual."
          ]
        }
      ]
    },
    {
      "id": "vascularisation",
      "title": "Vascularización",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Arterial: ramas terminales de las arterias pudendas internas, que a su vez son ramas anteriores de las arterias ilíacas internas.",
            "Venosa: las venas pudendas internas acompañan a las arterias pudendas internas y drenan principalmente hacia el sistema ilíaco interno. Desde la perspectiva clínica de la ROP, pueden tenerse en cuenta las congestiones venosas pélvicas, pero no conviene asimilar directamente el drenaje venoso de los órganos eréctiles al sistema porta hepático."
          ]
        }
      ]
    },
    {
      "id": "innervation",
      "title": "Inervación",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Inervación somática: el nervio pudendo es el principal nervio del periné. Asegura la sensibilidad de los órganos genitales externos y el control motor somático de la musculatura perineal estriada, en particular de los músculos bulboesponjosos e isquiocavernosos.",
            "Inervación autónoma:",
            "Sistema parasimpático sacro de S2 a S4: participa en la vasodilatación y en el llenado de los tejidos eréctiles;",
            "Sistema simpático toracolumbar de L1 a L3: interviene especialmente en la fase de emisión de la eyaculación."
          ]
        }
      ]
    },
    {
      "id": "physiologie",
      "title": "Fisiología",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erección: bajo la influencia parasimpática sacra de S2 a S4, la vasodilatación de las arterias pudendas internas y de sus ramas aumenta el aporte sanguíneo a los tejidos eréctiles del pene y del clítoris. En la mujer, la excitación sexual se acompaña también de congestión vulvovaginal y de lubricación vaginal.",
            "Eyaculación: comprende una fase de emisión, predominantemente simpática, con progresión de las secreciones de las vías genitales hacia la uretra prostática, seguida de una fase de expulsión, en la que intervienen en particular las contracciones rítmicas de los músculos bulboesponjosos bajo control somático del nervio pudendo.",
            "Orgasmo: acción del simpático en la mujer y en el hombre."
          ]
        }
      ]
    },
    {
      "id": "pathologies-courantes",
      "title": "Patologías frecuentes",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Déficit gonadal: disminución de la fertilidad, descenso de la libido, dismenorrea, premenopausia y andropausia precoces (véase Capítulo 5 Mecanismo del estrés, página…)",
            "En el hombre: los trastornos sexuales son frecuentes y variados: pérdida de la libido, disfunciones sexuales, disfunción eréctil, eyaculación precoz, incapacidad para eyacular y dolor durante las relaciones sexuales. (véase Capítulo 16 Riñones, vascularización venosa, riñón izquierdo, página…)",
            "En la mujer: los trastornos sexuales también son frecuentes y variados: falta de interés sexual (baja libido), sequedad vaginal, dolor durante las relaciones sexuales, vaginismo (espasmos dolorosos de la musculatura vaginal) y anorgasmia.",
            "Causas orgánicas: diabetes, hiperlipidemia, hipertensión arterial, insuficiencia coronaria, accidente cerebrovascular y obesidad.",
            "Causas psicoemocionales: véase Relaciones visceroemocionales)."
          ]
        },
        {
          "type": "xref",
          "label": "Ver la referencia en el capítulo 5",
          "text": "Mecanismo de estrés",
          "href": "/lecture/chapitre-5?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#p-presentation-0"
        },
        {
          "type": "xref",
          "label": "Ver la referencia en el capítulo 16",
          "text": "Riñones, drenaje venoso, riñón izquierdo",
          "href": "/lecture/chapitre-16?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#sec-vascularisation"
        }
      ]
    },
    {
      "id": "relations-viscero-somatiques",
      "title": "Relaciones viscerosomáticas",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Sacro, cóccix"
          ]
        }
      ]
    },
    {
      "id": "relations-viscero-emotionnelles",
      "title": "Relaciones visceroemocionales",
      "blocks": [
        {
          "type": "para",
          "text": "(Véanse los capítulos sobre los órganos genitales femeninos, página… y masculinos, página…)"
        },
        {
          "type": "para",
          "text": "No siempre es fácil distinguir entre causas físicas y causas psicoemocionales, pero estas últimas son frecuentes y variadas en ambos sexos."
        },
        {
          "type": "para",
          "text": "En el hombre, y más aún en la mujer, la sexualidad se vive tanto en la mente y en la esfera afectiva como en el cuerpo. Los factores de estrés psicoemocional son obstáculos para una sexualidad plena: pérdida de autoestima, ansiedad, estrés, ansiedad de rendimiento, depresión, conflictos de pareja, tabúes religiosos o educativos y traumatismos sexuales sufridos en el pasado, como incesto, violación, tocamientos y otros abusos."
        }
      ]
    },
    {
      "id": "zones-reflexes-podales",
      "title": "Zonas reflejas podales",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Nervio pudendo (S2 a S4): cara lateral del talón, plexo sacro, espina isquiática;,",
            "Cara plantar del talón:",
            "Nervio pudendo (continuación): borde medial de la rama isquiopubiana",
            "Cuerpos cavernosos y músculos isquiocavernosos: borde medial de las ramas isquiopubianas;",
            "Órganos eréctiles: extremo medial de la rama iliopubiana (borde anterior del talón);",
            "Ligamentos sacrotuberoso y sacroespinoso: borde posterior de los talones;",
            "Sistema límbico;",
            "Nervio olfatorio I."
          ]
        }
      ]
    }
  ]
}
