import type { Chapter } from './types'

// Italian content for chapter 21
export const chapter21It: Chapter = {
  "slug": "chapter-21",
  "number": "21",
  "title": "Sistema erettile maschile e femminile",
  "sections": [
    {
      "id": "presentation",
      "title": "Presentazione",
      "blocks": [
        {
          "type": "para",
          "text": "L’eccitazione sessuale provoca l’erezione degli organi erettili, cioè clitoride e pene, nonché la lubrificazione vaginale, permettendo la copulazione."
        }
      ]
    },
    {
      "id": "situation",
      "title": "Situazione",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Gli organi genitali esterni dell’uomo e della donna si inseriscono sulla faccia inferiore del triangolo urogenitale del perineo, con il quale sono intimamente solidali."
          ]
        }
      ]
    },
    {
      "id": "anatomie",
      "title": "Anatomia",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Organi erettili:",
            "Corpi cavernosi: organi erettili pari, hanno le loro inserzioni nella parte prossimale dell’arco pubico. Sono chiamati pilastri del clitoride o del pene. Alle estremità distali si riuniscono per formare il corpo del clitoride nella donna e le porzioni dorsali del pene nell’uomo.",
            "Bulbi del vestibolo: nella donna sono due organi erettili situati ai lati dell’orifizio vaginale. Partecipano al turgore erettile della vulva e sono omologhi al corpo spongioso maschile, senza formare direttamente il glande del clitoride.",
            "Corpo spongioso: nell’uomo è impari, mediano, e nasce con un rigonfiamento, il bulbo del pene, fissato alla membrana perineale. Termina nel glande. Contiene l’uretra. Contribuisce a espellere il contenuto dell’uretra durante la minzione e durante l’eiaculazione dello sperma nell’uomo.",
            "Muscoli degli organi erettili:",
            "Muscoli ischiocavernosi: pari, si inseriscono sul margine mediale dei rami ischiopubici e sui pilastri del clitoride e del pene. Contraendosi, propulsano il sangue dai pilastri verso il corpo del pene e del clitoride.",
            "Muscoli bulbospongiosi: pari, si inseriscono sul perineo, sul bulbo del vestibolo nella donna e sul corpo spongioso nell’uomo. Contraendosi, spingono il sangue verso il pene e il clitoride. Nell’uomo facilitano lo svuotamento dell’uretra dopo la minzione e l’eiaculazione dello sperma mediante le loro contrazioni riflesse.",
            "Legamenti del pene: il pene è fissato da due legamenti:",
            "Legamento sospensore: è fissato superiormente alla sinfisi pubica e alla linea alba della parete addominale anteriore;",
            "Legamento fundiforme: è fissato alla linea alba.",
            "Ghiandole vestibolari di Bartolino: nella donna sono situate nel perineo, posteriormente ai bulbi vestibolari. Partecipano alla lubrificazione dell’ingresso vaginale. La lubrificazione vaginale dipende anche dalla trasudazione vascolare attraverso la parete vaginale durante l’eccitazione sessuale.",
            "Ghiandole bulbouretrali di Cowper: nell’uomo sono situate nel bulbo del perineo. Secernono un liquido alcalino che protegge gli spermatozoi durante il loro passaggio attraverso l’uretra e la vagina nel corso dell’eccitazione sessuale."
          ]
        }
      ]
    },
    {
      "id": "vascularisation",
      "title": "Vascolarizzazione",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Arteriosa: rami terminali delle arterie pudende interne, a loro volta rami anteriori delle arterie iliache interne.",
            "Venosa: le vene pudende interne accompagnano le arterie pudende interne e drenano principalmente nel sistema iliaco interno. Nella lettura clinica ROP si possono prendere in considerazione le congestioni venose pelviche, ma non si deve assimilare direttamente il drenaggio venoso degli organi erettili al sistema portale epatico."
          ]
        }
      ]
    },
    {
      "id": "innervation",
      "title": "Innervazione",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Innervazione somatica: il nervo pudendo è il principale nervo del perineo. Assicura la sensibilità degli organi genitali esterni e il comando somatico dei muscoli perineali striati, in particolare dei muscoli bulbospongiosi e ischiocavernosi.",
            "Innervazione autonoma:",
            "Sistema parasimpatico sacrale da S2 a S4: partecipa alla vasodilatazione e al riempimento dei corpi erettili;",
            "Sistema simpatico toracolombare da L1 a L3: interviene in particolare nella fase di emissione dell’eiaculazione."
          ]
        }
      ]
    },
    {
      "id": "physiologie",
      "title": "Fisiologia",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Erezione: sotto l’influenza parasimpatica sacrale da S2 a S4, la vasodilatazione delle arterie pudende interne e dei loro rami aumenta l’afflusso sanguigno nei corpi erettili del pene e del clitoride. Nella donna, l’eccitazione sessuale si accompagna anche a congestione vulvovaginale e lubrificazione vaginale.",
            "Eiaculazione: comprende una fase di emissione, prevalentemente simpatica, con progressione delle secrezioni delle vie genitali verso l’uretra prostatica, seguita da una fase di espulsione, che mobilizza in particolare le contrazioni ritmiche dei muscoli bulbospongiosi sotto controllo somatico pudendo.",
            "Orgasmo: azione del simpatico nella donna e nell’uomo."
          ]
        }
      ]
    },
    {
      "id": "pathologies-courantes",
      "title": "Patologie comuni",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Deficit gonadico: riduzione della fertilità, calo della libido, dismenorrea, premenopausa e andropausa precoci (cfr. Capitolo 5 Meccanismo dello stress, pagina…)",
            "Nell’uomo: i disturbi sessuali sono numerosi e frequenti: perdita della libido, défaillance sessuali, problemi di erezione, eiaculazione precoce, incapacità di eiaculare, dolore durante i rapporti. (cfr. Capitolo 16 Reni, vascolarizzazione venosa, rene sinistro, pagina…)",
            "Nella donna: anche i disturbi sessuali sono numerosi e frequenti: scarso interesse sessuale (bassa libido), secchezza vaginale, dolore durante i rapporti, vaginismo (spasmi dolorosi dei muscoli vaginali), anorgasmia.",
            "Cause organiche: diabete, iperlipidemia, ipertensione arteriosa, insufficienza coronarica, ictus, obesità.",
            "Cause psico-emozionali: cfr. Relazioni viscero-emozionali)."
          ]
        },
        {
          "type": "xref",
          "label": "Vedi il riferimento nel capitolo 5",
          "text": "Meccanismo di stress",
          "href": "/lecture/chapitre-5?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#p-presentation-0"
        },
        {
          "type": "xref",
          "label": "Vedi il riferimento nel capitolo 16",
          "text": "Reni, drenaggio venoso, rene sinistro",
          "href": "/lecture/chapitre-16?xrefBack=%2Flecture%2Fchapitre-21%23p-pathologies-courantes-0&xrefBackLabel=Retour%20au%20chapitre%2021#sec-vascularisation"
        }
      ]
    },
    {
      "id": "relations-viscero-somatiques",
      "title": "Relazioni viscero-somatiche",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Sacro, coccige"
          ]
        }
      ]
    },
    {
      "id": "relations-viscero-emotionnelles",
      "title": "Relazioni viscero-emozionali",
      "blocks": [
        {
          "type": "para",
          "text": "(Cfr. i capitoli sugli organi genitali femminili, pagina… e maschili, pagina…)"
        },
        {
          "type": "para",
          "text": "Non è sempre facile distinguere le cause fisiche da quelle psico-emozionali, ma queste ultime sono numerose e frequenti in entrambi i sessi."
        },
        {
          "type": "para",
          "text": "Nell’uomo, e ancor più nella donna, la sessualità si svolge tanto nella mente e nel cuore quanto nel corpo. Gli stress psico-emozionali sono ostacoli a una sessualità appagante: perdita dell’autostima, ansia, stress, angoscia da prestazione, depressione, conflitti nella coppia, tabù religiosi o educativi, traumi sessuali vissuti in passato (incesto, stupro, molestie ecc)."
        }
      ]
    },
    {
      "id": "zones-reflexes-podales",
      "title": "Zone riflesse podaliche",
      "blocks": [
        {
          "type": "bullets",
          "items": [
            "Nervo pudendo (S2 a S4): faccia laterale del tallone, plesso sacrale, spina ischiatica;,",
            "Faccia plantare del tallone:",
            "Nervo pudendo (seguito): margine mediale del ramo ischiopubico",
            "Corpi cavernosi e muscoli ischiocavernosi: margine mediale dei rami ischiopubici;",
            "Organi erettili: estremità mediale del ramo iliopubico (bordo anteriore del tallone);",
            "Legamenti sacrotuberoso e sacrospinoso: bordo posteriore dei talloni;",
            "Cervello limbico;",
            "Nervo olfattivo I."
          ]
        }
      ]
    }
  ]
}
