...
// Classic single-column reading. The synchronized reader at /lecture/chapitre-19 is
...
export default async function Chapitre19ClassicPage({
  searchParams,
}: {
  searchParams: Promise<{ lang?: string }>
}) {
  ...
  const { lang: langParam } = await searchParams
  const lang = await getServerLang(langParam)
  const { chapter, contentLang } = getChapter('chapter-19', lang)
  const syncHref = `/lecture/chapitre-19?lang=${lang}`
