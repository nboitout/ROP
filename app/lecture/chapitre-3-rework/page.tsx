...
export const metadata: Metadata = { title: 'Chapitre 3 — Nouvelle édition privée — R.O.P.', description: 'Version privée du chapitre 3 sur le système nerveux central.', robots: { index: false, follow: false } }
...
export default async function Chapter3ReworkPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return <SlideSyncReader chapter={chapter3ReworkFr} bookTitle={`${translations[lang].reader.bookTitle} · Nouvelle édition privée`} slides={chapter3ReworkSlides} anchors={chapter3ReworkSlideAnchors} backHref={draftBackHref(isAdmin)} classicHref={`/chapitre-3-rework?lang=${lang}`} />
}
