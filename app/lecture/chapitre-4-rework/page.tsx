...
import { chapter4ReworkSlides, chapter4ReworkSlideAnchors } from '@/content/chapter4-rework.slidesync'
import { DRAFT_KEY, draftBackHref } from './access'

export const metadata: Metadata = { title: 'Chapitre 4 — Nouvelle édition privée — R.O.P.', description: 'Version privée reconstruite du chapitre 4 sur le système nerveux autonome.', robots: { index: false, follow: false } }
...
export default async function Chapter4ReworkPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  return <SlideSyncReader chapter={chapter4ReworkFr} bookTitle={`${translations[lang].reader.bookTitle} · Nouvelle édition privée`} slides={chapter4ReworkSlides} anchors={chapter4ReworkSlideAnchors} backHref={draftBackHref(isAdmin)} classicHref={`/chapitre-4-rework?lang=${lang}`} />
}
